...
bot.command("start", (ctx) => {
  ctx.reply(
    `Вот что я умею:<br><br>\\-Просто отправь мне сообщение, чтобы добавить новую задачу.<br>\\-Используй команду /viewtasks, чтобы посмотреть все свои задачи.<br>\\-Используй команду /deletealltasks, чтобы удалить все задачи.<br><br>\\-Каждая задача имеет следующие кнопки:<br>\\-"Сделано" / "Не сделано" для изменения статуса задачи.<br>\\-"Изменить" для редактирования задачи.<br>\\-"Удалить" для удаления задачи.`,
    { parse_mode: "HTML" }
  );
});
