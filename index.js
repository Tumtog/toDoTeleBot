...
bot.command("start", (ctx) => {
  ctx.reply(
    `Привет\\! Я твой персональный бот для управления задачами\\.\\n\\n` +
    `Вот что я умею:\\n` +
    `- Просто отправь мне сообщение, чтобы добавить новую задачу\\.\\n` +
    `- Используй команду /viewtasks, чтобы посмотреть все свои задачи\\.\\n` +
    `- Используй команду /deletealltasks, чтобы удалить все задачи\\.\\n\\n` +
    `Каждая задача имеет следующие кнопки:\\n` +
    `- "Сделано" / "Не сделано" для изменения статуса задачи\\.\\n` +
    `- "Изменить" для редактирования задачи\\.\\n` +
...
bot.on("message:text", async (ctx) => {
  ...
  // Проверка, если бот находится в группе и сообщение не содержит команду
  if (chatType === "group" || chatType === "supergroup") {
    if (!ctx.message.text.startsWith("/")) {
      return; // Игнорировать сообщение, если оно не начинается с "/"
    }
  }

  // Обработка команд и сообщений, как обычно
  const userId = ctx.from.id;
